...
export function TopologicalSortCodePanel({ codeSnippets, currentLine, defaultLanguage = "JavaScript" }: TopologicalSortCodePanelProps) {
  ...
  const handleCopyCode = () => {
    ...
    if (codeToCopy && selectedLanguage !== 'Info') {
      navigator.clipboard.writeText(codeToCopy)
        .then(() => {
          toast({ title: `${selectedLanguage} Topological Sort Code Copied!` });
        })
        .catch(err => {
    ...
    <Card className="shadow-lg rounded-lg h-[400px] md:h-[500px] lg:h-[550px] flex flex-col">
      <CardHeader className="flex flex-row items-center justify-between pb-2 shrink-0">
        <CardTitle className="font-headline text-xl text-primary dark:text-accent flex items-center">
            <Code2 className="mr-2 h-5 w-5" /> Topological Sort Code (Kahn's)
        </CardTitle>
