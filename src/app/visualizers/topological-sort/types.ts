...
// src/app/visualizers/topological-sort/types.ts
...
export interface GraphNode {
  ...
  color?: string;
  isStartNode?: boolean;
  distance?: number | typeof Infinity; // Re-using distance to show in-degree for TopoSort
}
...
export interface GraphAlgorithmStep {
  ...
  message?: string;
  auxiliaryData?: Array<{
    type: 'queue' | 'set'; // For Kahn's: queue for processing, set for sorted order and in-degrees
    label: string;
    values: any; // string[] or Record<string, number> for in-degrees
  }>;
  array?: number[] | string[];
