...
export const CIRCULAR_LINKED_LIST_OPERATIONS_CODE: Record<string, Record<string, string[]>> = {
  JavaScript: {
    ...
    insertAtPosition: [
      ...
      "  else if (count === position && current === this.head) { prev.next = newNode; newNode.next = this.head; }",
      "}",
    ],
    deleteAtPosition: [
      ...
      "    count++;",
      "  } while (current !== this.head && count < position);",
      "  if (current !== this.head && count === position && prev) { prev.next = current.next; }",
  ...
  Python: {
    ...
    insertAtPosition: [
      ...
      "        new_node = Node(data)",
      "        if position == 0: self.insert_head(data); return",
      "        if not self.head: return",
      "        current, prev, count = self.head, None, 0",
      "        while count < position:",
      "            prev = current; current = current.next; count += 1",
    ...
    deleteAtPosition: [
      ...
      "            else: tail.next = self.head.next; self.head = self.head.next",
      "            return",
      "        while count < position:",
      "            prev = current; current = current.next; count += 1",
      "            if current == self.head: return # Position out of bounds",
