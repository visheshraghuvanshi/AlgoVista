          ...
          {positionedNodes.length === 0 ? (
            ...
            <svg width="100%" height="100%" viewBox={viewBox} preserveAspectRatio="xMidYMid meet">
              ...
              {positionedNodes.map((node) => {
                ...
                if (isAuxPointed && !node.isHead && !node.isSlow && !node.isFast) {
              ...
              {Object.entries(auxiliaryPointers).map(([key, nodeIdStr], index) => {
                ...
                return (
                  <g key={`aux-${key}-${nodeIdStr}`}>
                    <text x={targetNode.x!} y={targetNode.y! - NODE_HEIGHT / 2 - 5 - (Object.keys(auxiliaryPointers).indexOf(key) * 10) % 20} 
                          textAnchor="middle" fontSize="10" fill="hsl(var(--foreground))" className="font-mono">
                      {key} ({auxiliaryPointers[key+'_val'] || targetNode.value})
                    </text>
                    <line x1={targetNode.x!} y1={targetNode.y! - NODE_HEIGHT / 2 - 2 - (Object.keys(auxiliaryPointers).indexOf(key) * 10) % 20} 
