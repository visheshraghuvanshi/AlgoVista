...
export function LCSVisualizationPanel({ step }: LCSVisualizationPanelProps) {
  if (!step || !step.dpTable || !Array.isArray(step.dpTable) || !Array.isArray(step.dpTable[0])) {
    return (
      <Card className="flex-1 shadow-lg rounded-lg overflow-hidden h-[400px] md:h-[500px] lg:h-[550px] flex flex-col">
        <CardHeader><CardTitle className="font-headline text-xl text-primary dark:text-accent">LCS DP Table</CardTitle></CardHeader>
  ...
  const lcsString = auxiliaryData?.lcsString as string | undefined;
  ...
  return (
    <Card className="flex-1 shadow-lg rounded-lg overflow-hidden h-auto md:h-[500px] lg:h-[550px] flex flex-col">
      <CardHeader className="pb-2">
        <CardTitle className="font-headline text-xl text-primary dark:text-accent">Longest Common Subsequence DP Table</CardTitle>
        {message && <p className="text-xs text-muted-foreground">{message}</p>}
        {resultValue !== undefined && <p className="text-sm font-bold">LCS Length: {resultValue}</p>}
        {lcsString !== undefined && <p className="text-sm font-bold">LCS String: "{lcsString}"</p>}
      </CardHeader>
