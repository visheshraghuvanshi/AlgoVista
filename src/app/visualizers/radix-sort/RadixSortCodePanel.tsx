...
export function RadixSortCodePanel({ currentLine }: RadixSortCodePanelProps) {
  ...
  // Using RADIX_SORT_CODE_SNIPPETS directly as it's now defined in this file
  const languages = useMemo(() => Object.keys(RADIX_SORT_CODE_SNIPPETS), []);
  
  const [selectedLanguage, setSelectedLanguage] = useState<string>(() => {
    const initialLangs = Object.keys(RADIX_SORT_CODE_SNIPPETS);
  ...
  const handleCopyCode = () => {
    const codeToCopy = RADIX_SORT_CODE_SNIPPETS[selectedLanguage as keyof typeof RADIX_SORT_CODE_SNIPPETS]?.join('\n') || '';
              ...
              <TabsContent key={lang} value={lang} className="m-0 flex-grow overflow-hidden flex flex-col">
                <ScrollArea className="flex-1 overflow-auto border-t bg-muted/20 dark:bg-muted/5">
                  <pre className="font-code text-sm p-4 whitespace-pre-wrap overflow-x-auto">
                    {(RADIX_SORT_CODE_SNIPPETS[lang as keyof typeof RADIX_SORT_CODE_SNIPPETS] || []).map((line, index) => (
                      <div
