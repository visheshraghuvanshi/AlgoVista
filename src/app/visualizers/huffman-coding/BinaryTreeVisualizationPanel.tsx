    ...
    <Card className="flex-1 shadow-lg rounded-lg overflow-hidden h-[400px] md:h-[500px] lg:h-[600px] flex flex-col">
      ...
      <CardContent className="flex flex-col flex-grow p-4 space-y-4 overflow-hidden">
            ...
            <svg width="100%" height="100%" viewBox={viewBox} preserveAspectRatio="xMidYMid meet">
              <g>
                ...
                {nodes.map((node) => {
                  ...
                  // For Huffman, node.value already contains 'char:freq' or 'Σ:freq'
                  const nodeLabel = node.value; 
                  const textColor = node.textColor || "hsl(var(--primary-foreground))"; // Default text color for highlighted nodes
                  ...
                  } else if (node.color === "hsl(var(--accent))") { // Example: found highlight
                  ...
                  return (
                    <g key={node.id} transform={`translate(${node.x},${node.y})`}>
                      ...
                      <text
                        ...
                        textAnchor="middle"
                        dy=".3em"
                        fontSize="9" // Slightly smaller for 'char:freq'
        ...
        <div className="flex-shrink-0 p-2 border rounded-md bg-background">
            <p className="font-semibold text-muted-foreground text-sm">Current Path (for Code Gen):</p>
