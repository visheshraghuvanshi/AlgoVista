...
interface LinkedListVisualizationPanelProps {
  ...
  headId?: string | null;
  tailId?: string | null; // Added for DLL
...
const NODE_SPACING_X = 90; // Increased spacing for prev pointer
const POINTER_OFFSET = 10; // For prev pointer display
...
export function LinkedListVisualizationPanel({
  ...
  const positionedNodes = nodes.map((node, index) => ({
    ...
    y: node.y ?? SVG_PADDING + NODE_HEIGHT * 1.5, // Center vertically with more space for aux pointers
  }));
  ...
  const svgHeight = Math.max(150, SVG_PADDING * 2 + NODE_HEIGHT * 2.5 + Object.keys(auxiliaryPointers).length * 25 + (message ? 30: 0) ); // Increased height
        ...
        <div className="flex-grow w-full border rounded-md bg-muted/10 dark:bg-muted/5 flex items-center justify-center overflow-auto relative p-2">
          {positionedNodes.length === 0 ? (
            ...
            <svg width="100%" height="100%" viewBox={viewBox} preserveAspectRatio="xMidYMid meet">
              <defs>
                <marker
                  id="ll-arrowhead" markerWidth="10" markerHeight="7"
                  refX="2" refY="1.75" orient="auto" markerUnits="strokeWidth">
                  <path d="M0,0 L0,3.5 L2.5,1.75 z" fill="hsl(var(--muted-foreground))" />
                </marker>
                 <marker
                  id="ll-arrowhead-active" markerWidth="10" markerHeight="7"
                  refX="2" refY="1.75" orient="auto" markerUnits="strokeWidth">
              ...
              {positionedNodes.map((node) => {
                ...
                if (nextNode) {
                  ...
                  return (
                    <line
                      key={`next-${node.id}`} x1={node.x! + NODE_WIDTH / 2} y1={node.y! - POINTER_OFFSET / 2}
                      x2={nextNode.x! - NODE_WIDTH / 2 - 5} y2={nextNode.y! - POINTER_OFFSET / 2}
                      stroke={isNextActive ? "hsl(var(--primary))" : "hsl(var(--muted-foreground))"}
                      strokeWidth="1.5" markerEnd={isNextActive ? "url(#ll-arrowhead-active)" : "url(#ll-arrowhead)"}
                    />
                  );
                }
                  ...
                  if(headNodeObj) {
                    return (
                       <path key={`circ-${node.id}`}
                          d={`M ${node.x! + NODE_WIDTH/2},${node.y!} Q ${node.x! + NODE_WIDTH/2 + 20},${node.y! - NODE_HEIGHT*0.75} ${headNodeObj.x! - NODE_WIDTH/2 - 5},${headNodeObj.y! - NODE_HEIGHT*0.3}`}
                          stroke="hsl(var(--muted-foreground))" strokeWidth="1.5" fill="none" markerEnd="url(#ll-arrowhead)" />
                    );
                  }
                }
                return null;
              ...
              {listType === 'doubly' && positionedNodes.map((node) => {
                ...
                if (prevNode) {
                  ...
                  return (
                    <line
                      key={`prev-${node.id}`} x1={node.x! - NODE_WIDTH / 2} y1={node.y! + POINTER_OFFSET / 2}
                      x2={prevNode.x! + NODE_WIDTH / 2 + 5} y2={prevNode.y! + POINTER_OFFSET / 2}
              ...
              {Object.entries(auxiliaryPointers).map(([key, nodeIdStr], index) => {
                ...
                return (
                  <g key={`aux-${key}-${nodeIdStr}`}>
                    <text x={targetNode.x!} y={targetNode.y! - NODE_HEIGHT / 2 - 10 - (Object.keys(auxiliaryPointers).filter(k => k !== 'slow' && k !== 'fast').indexOf(key) * 12)} 
                          textAnchor="middle" fontSize="10" fill="hsl(var(--foreground))" className="font-mono">
                      {key} ({auxiliaryPointers[key+'_val'] || targetNode.value})
                    </text>
                    <line x1={targetNode.x!} y1={targetNode.y! - NODE_HEIGHT / 2 - 7 - (Object.keys(auxiliaryPointers).filter(k => k !== 'slow' && k !== 'fast').indexOf(key) * 12)} 
