...
const LEVEL_MARGIN = 60; // Increased for better tree separation
const SVG_PADDING_X = 20;
const SVG_PADDING_Y = 30;

// Colors matching PRD concepts
const NODE_COLORS = {
  ...
  highlighted: "bg-yellow-400/40 text-yellow-900 dark:text-yellow-300 border-yellow-500/50", // Yellow highlight
  updated: "bg-red-500/40 text-red-800 dark:text-red-300 border-red-500/50", // Red for updated
...
export function SegmentTreeVisualizationPanel({ 
  data, // This is the segment tree array
  activeIndices = [],
  originalInputArray = [],
  processingSubArrayRange = null,
  originalArraySize = 0, // N, the size of the original input array
  ...
  if (tree.length > 1 && n > 0) { // Tree array starts at index 1 for root
    const treeDepth = Math.ceil(Math.log2(n)) + 1;
    const maxLeafNodesAtBottom = Math.pow(2, treeDepth -1); // Max leaves if perfect tree
    const svgTreeWidth = Math.max(300, maxLeafNodesAtBottom * (ELEMENT_BOX_SIZE + ELEMENT_MARGIN * 3)); // More spacing


    const calculateTreePositions = (nodeTreeIndex: number, currentDepth: number, x: number, y: number, horizontalSpan: number) => {
      if (nodeTreeIndex >= tree.length || nodeTreeIndex === 0 || tree[nodeTreeIndex] === undefined) return;
      
      const isLeaf = nodeTreeIndex >= n && nodeTreeIndex < 2 * n; // Iterative leaves are at [n, 2n-1]
      ...
      if (!isLeaf) {
        ...
        if (leftChildIndex < tree.length && tree[leftChildIndex] !== undefined) {
          ...
          calculateTreePositions(leftChildIndex, currentDepth + 1, x - horizontalSpan / 2, childY, horizontalSpan / 2);
        }
        if (rightChildIndex < tree.length && tree[rightChildIndex] !== undefined) {
          ...
          calculateTreePositions(rightChildIndex, currentDepth + 1, x + horizontalSpan / 2, childY, horizontalSpan / 2);
        }
      }
    };
    
    if (tree.length > 1) { // Root is at index 1
  ...
  const getElementColorClasses = (treeIndex: number, isLeaf: boolean) => {
    ...
    // Specific logic for updated node might need to come from auxData if step is 'update'
    // For now, using leaf/internal colors.
    ...
    <Card className="flex-1 shadow-lg rounded-lg overflow-hidden h-[500px] md:h-[600px] lg:h-[700px] flex flex-col">
      ...
      <CardContent className="flex flex-col flex-grow p-2 space-y-2 bg-muted/10 dark:bg-muted/5 overflow-hidden">
        ...
        <div className="flex-grow w-full border rounded-md bg-background overflow-auto">
          {nodePositions.length === 0 && n > 0 && ( // If tree built but no nodes (error case)
