...
export function VisualizationPanel({ 
  ...
  const getBarColor = (index: number) => {
    ...
    if (processingSubArrayRange && index >= processingSubArrayRange[0] && index <= processingSubArrayRange[1]) {
      return "bg-primary"; // Current window being processed
    }
    // activeIndices can be used to highlight start/end pointers if not covered by processingSubArrayRange
    if (activeIndices.includes(index)) return "bg-accent"; 
    return "bg-secondary"; 
  };
