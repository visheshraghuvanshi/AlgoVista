...
interface SubarraySumCodePanelProps {
  codeSnippets: Record<SubarraySumProblemType, string[]>;
  currentLine: number | null;
  selectedProblem: SubarraySumProblemType;
}

export function SubarraySumCodePanel({ codeSnippets, currentLine, selectedProblem }: SubarraySumCodePanelProps) {
  const { toast } = useToast();
  
  const codeToDisplay = codeSnippets[selectedProblem] || [];
  ...
  const handleCopyCode = () => {
    ...
    if (codeString) {
      navigator.clipboard.writeText(codeString)
        .then(() => toast({ title: `${problemLabel} Code Copied!` }))
        ...
        <ScrollArea className="flex-1 overflow-auto border-t bg-muted/20 dark:bg-muted/5">
          <pre className="font-code text-sm p-4">
            {codeToDisplay.map((line, index) => (
              <div key={`${selectedProblem}-line-${index}`}
            ...
            {codeToDisplay.length === 0 && <p className="text-muted-foreground">Select a problem type to view code.</p>}
          </pre>
        </ScrollArea>
      </CardContent>
    </Card>
