...
interface GraphCycleDetectionCodePanelProps {
  codeSnippets: { 
    undirected: { [language: string]: string[] },
    directed: { [language: string]: string[] }
  };
...
export function GraphCycleDetectionCodePanel({ codeSnippets, currentLine, graphType, defaultLanguage = "JavaScript" }: GraphCycleDetectionCodePanelProps) {
  const { toast } = useToast();
  
  const currentGraphTypeSnippets = codeSnippets[graphType] || {};
  const languages = useMemo(() => Object.keys(currentGraphTypeSnippets), [currentGraphTypeSnippets]);
  ...
  useEffect(() => { // Reset language if graphType changes and current lang not available for new type
    const currentProblemLangs = Object.keys(codeSnippets[graphType] || {});
    const initialLangForType = currentProblemLangs.includes(defaultLanguage) ? defaultLanguage : (currentProblemLangs[0] || 'Info');
    if(selectedLanguage !== initialLangForType) {
        setSelectedLanguage(initialLangForType);
    }
  }, [graphType, defaultLanguage, selectedLanguage, codeSnippets]);
  ...
  const handleCopyCode = () => {
    const codeToCopy = currentGraphTypeSnippets[selectedLanguage]?.join('\n') || '';
    if (codeToCopy && selectedLanguage !== 'Info') {
      navigator.clipboard.writeText(codeToCopy)
        .then(() => {
          toast({ title: `${selectedLanguage} Code for ${graphType} Cycle Detection Copied!` });
        })
        .catch(err => {
  ...
  const currentCodeLines = useMemo(() => {
    return selectedLanguage === 'Info' ? [] : (currentGraphTypeSnippets[selectedLanguage] || []);
  }, [selectedLanguage, currentGraphTypeSnippets]);

  const effectiveTabValue = languages.length === 0 ? 'Info' : (languages.includes(selectedLanguage) ? selectedLanguage : getInitialLanguage());
  const panelTitle = graphType.charAt(0).toUpperCase() + graphType.slice(1) + " Cycle Detection";
        ...
        {languages.length > 0 ? (
              ...
              <TabsContent key={`${lang}-content`} value={lang} className="m-0 flex-grow overflow-hidden flex flex-col">
                <ScrollArea key={`${lang}-scrollarea`} className="flex-1 overflow-auto border-t bg-muted/20 dark:bg-muted/5">
                  <pre className="font-code text-sm p-4 whitespace-pre-wrap overflow-x-auto">
                    {(currentGraphTypeSnippets[lang] || []).map((line, index) => (
                      <div
                        key={`${lang}-line-${index}`}
          ...
          <div className="flex-grow overflow-hidden flex flex-col">
            <ScrollArea className="flex-1 overflow-auto border-t bg-muted/20 dark:bg-muted/5">
              <pre className="font-code text-sm p-4 whitespace-pre-wrap overflow-x-auto">
                 <p className="text-muted-foreground p-4">No code snippets available for this graph type.</p>
              </pre>
            </ScrollArea>
          </div>
        )}
