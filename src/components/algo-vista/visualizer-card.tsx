...
export function VisualizerCard({ algorithm }: VisualizerCardProps) {
  const difficultyColors: Record<AlgorithmMetadata['difficulty'], string> = {
    ...
    Medium: "bg-yellow-500/20 text-yellow-700 dark:text-yellow-400 border-yellow-500/50", // This was yellow-500, ensuring it's distinct if used on yellow bg
  ...
  return (
    <Card className="flex flex-col h-full shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out transform hover:scale-[1.03] bg-accent text-accent-foreground dark:bg-primary dark:text-accent border-border rounded-xl overflow-hidden">
      ...
      <CardContent className="flex-grow space-y-3">
        ...
        {/* Description will use the card's default text color: text-accent-foreground (dark on yellow) in light, dark:text-accent (yellow on burgundy) in dark */}
